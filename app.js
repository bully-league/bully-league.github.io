/* The league PWA client — shared by every league, branded from site.json at runtime.
...
/* This shell is shared source — the bot copies site/ into EVERY league's repo — so
   the masthead can't name a league. site.json carries leagueName; index.html's own
   text is the fallback, which is what an older cached site.json still renders. */
function applyBranding(name) {
  if (!name) return;
  document.title = name;
  const main = document.querySelector(".wordmark-main");
  // The wordmark is a two-part block: a small "THE" eyebrow over the name. A league
  // whose name already starts with "The" hands that word to the eyebrow instead of
  // reading "THE THE ...".
  if (main) main.textContent = name.replace(/^the\s+/i, "").toUpperCase();
  // The home-screen label iOS shows under an installed PWA, and the description search
  // and link previews use. Both are shared-shell text that would otherwise carry the
  // first league's name onto every other league's site.
  const homeScreen = document.querySelector('meta[name="apple-mobile-web-app-title"]');
  if (homeScreen) homeScreen.setAttribute("content", name);
  const description = document.querySelector('meta[name="description"]');
  if (description) description.setAttribute("content", `Live standings, rankings, and league news for the ${name} Madden franchise.`);
}
